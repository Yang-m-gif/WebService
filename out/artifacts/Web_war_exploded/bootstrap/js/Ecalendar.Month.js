        ...
        _this.click(function(event){
        	...
        	if(config.disabled == disabled){
        		...
        		if(disabled == false){
            		...
            		$(".oper li:last-child").css("pointer-events","auto");
                    ...
                    var nowyear = disabled[0];
                    var nowmonth = disabled[1];
                    var nowday = disabled[2];

        			if(nowyear < getFullYear){

        				$(".oper li:last-child").css("pointer-events","none");
        				$("ul .prve").css("pointer-events","none");
        				$(".day li[data]").css("pointer-events","none");
        				$("ul .next").css("pointer-events","none");

        			//该条件刚好为跨年的时候
        			}else if(nowyear == (getFullYear+1) && (nowmonth+11) == getMonth){
                        $(".oper li:last-child").css("pointer-events","auto");
                        ...
                        $(".day li[data]").each(function(index){
                            var nowselected1 = $(".day li[data]").eq(index).text();
                            if(nowselected1<nowday){
                                $(".day li[data]").eq(index).css("pointer-events","none");
                            }
                        });
                        //遍历next大于穿入日期不能选
                        $("ul .next").each(function(index){
                            var nowselected5 = $("ul .next").eq(index).text();
                            if(nowselected5>nowday){
                                $("ul .next").eq(index).css("pointer-events","none");
                            }
                        });


                    }else if(nowyear == getFullYear && (nowmonth-1) == getMonth){
        				$(".oper li:last-child").css("pointer-events","auto");
        				$(".oper li:first-child").css("pointer-events","none");
        				$("ul .prve").css("pointer-events","none");
        				//小于当天禁止选中
                        ...
                        $("ul .next").each(function(index){
                        	...
                        	if(nowselected2>nowday){
                        		$("ul .next").eq(index).css("pointer-events","none");
                        	}
                        });
        			}else if(nowyear == getFullYear && nowmonth == getMonth){
        				$(".oper li:first-child").css("pointer-events","auto");
        				$(".oper li:last-child").css("pointer-events","none");
        				$("ul .next").css("pointer-events","none");
        				//alert(333);
